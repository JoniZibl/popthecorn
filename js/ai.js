...
var AI = (function () {
  ...
  function evaluate(s, me, ctx) {
    ...
    for (k = 0; k < nocc; k++) {
      ...
      if (t === T.king) {
        if (att > 0) sc[own] -= 9000;                     // Turm im Schlagbereich
      } else {
        sc[own] += VALUE[t];
        if (att > 0) {
          if (def === 0) sc[own] -= VALUE[t] * 0.50;      // hängt frei
          else if (att > def) sc[own] -= VALUE[t] * 0.28; // überzählig angegriffen
          else sc[own] -= VALUE[t] * 0.06;                // gedeckt, aber unter Druck
        }
      }
    }
    ...
    for (p = 0; p < np; p++) {
      ...
      if (wk < 0) {
        ...
        sc[p] += Math.max(0, 110 - 26 * bestTree) + (near < 6 ? near : 6) * 10;
      }
      ...
      sc[p] += evalSpots.length ? 20 + (evalSpots.length < 4 ? evalSpots.length : 4) * 7 : -90;
    }
