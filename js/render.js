...
var Render = (function () {
  ...
  // Silhouetten der Figuren (dunkler Kopf, farbiger Schaft) – Füße auf y = 0
  var HEADS = {
    king:      'M-9,-9 L-9,-20 L-4.5,-15 L0,-23 L4.5,-15 L9,-20 L9,-9 Z',
    worker:    'M0,-23 L8,-9 L-8,-9 Z',
    samurai:   'M-8,-9 L-8,-15 L-2,-20 L3,-16 L9,-23 L6,-14 L8,-9 Z',
    springer:  'M-7,-9 L-6,-16 L-1,-21 L8,-18 L3,-14 L6,-9 Z',
    legionaer: 'M-5,-9 L-6,-18 L0,-22 L6,-18 L5,-9 Z',
    archer:    'M-8,-9 L-6,-14 L-7,-18 L0,-23 L7,-18 L6,-14 L8,-9 Z',
    tangolin:  'M0,-23 L7,-16 L0,-9 L-7,-16 Z',
    zenturio:  'M-4,-23 L4,-23 L9,-18 L9,-13 L4,-8.5 L-4,-8.5 L-9,-13 L-9,-18 Z'
  };

  // Silhouetten für die flachen Symbole in Seitenleiste und Regelwerk
  var ICON_HEADS = {
  ...
  function pieceGlyph(g, cam, o, cx, cy, z, piece, color, k) {
    groundShadow(g, cam, o, cx, cy, z, SIZE * 0.34, 'piece-shadow');
    g.appendChild(el('polygon', {
      class: 'piece-base', points: pts(cam, disc(cx, cy, z + 0.6, SIZE * 0.30, 12), o)
    }));
    g.appendChild(el('polygon', {
      class: 'piece-rim', points: pts(cam, disc(cx, cy, z + 3.2, SIZE * 0.24, 12), o),
      fill: color
    }));
    var stand = el('g', { class: 'piece-stand', transform: 'scale(' + k.toFixed(3) + ')' });
    stand.appendChild(el('rect', {
      class: 'piece-stem', x: -3.6, y: -12, width: 7.2, height: 11, rx: 1.6, fill: color
    }));
  ...
  function ghost(view, board, key, type, color) {
    ...
    if (!fx) return null;
    var stand = el('g', { transform: 'scale(' + fx.k.toFixed(3) + ')' });
    stand.appendChild(el('rect', {
      class: 'piece-stem', x: -3.6, y: -12, width: 7.2, height: 11, rx: 1.6, fill: color
    }));
  ...
  /* Figuren-Symbol als HTML-String – für Regelkarten und Startbildschirm.
     Bewusst flach: als kleines Zeichen ist die Draufsicht klarer als ein
     perspektivischer Aufsteller. */
  function pieceIcon(type, color) {
    var hexPts = H.cornerPoints(22, 1);
    ...
    return '<svg class="piece-icon" viewBox="-26 -26 52 52" aria-hidden="true">' +
      '<polygon class="icon-hex" points="' + hexPts + '"/>' +
      '<ellipse class="icon-shadow" cx="0" cy="12" rx="10" ry="3.5"/>' +
      '<path class="icon-base" d="M-10,11 L-7,4 L7,4 L10,11 Z"/>' +
      '<rect class="piece-stem" x="-3.6" y="-6" width="7.2" height="11" rx="1.6" fill="' + color + '"/>' +
      '<path class="piece-head" d="' + ICON_HEADS[type] + '" stroke="' + color + '"/>' +
      '</svg>';
  }
